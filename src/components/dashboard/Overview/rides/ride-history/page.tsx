...
import { ChevronDown, Filter, Download, Eye, Trash2, Search, X, AlertTriangle } from 'lucide-react';
...
const generateMockRides = () => {
  ...
  const locations = [
    'Kimihurura', 'Rebero', 'Kigali City Center', 'Nyarutarama', 
    'Gacuriro', 'Kisimenti', 'Kagugu', 'Remera'
  ];
  const drivers = [
    'Adeline KARAKE', 'Jean MUTABAZI', 'Marie UWIMANA', 
    'Claude NDAYISHIMIYE', 'Patrick NIYONZIMA'
  ];
...
const DeleteConfirmationModal = ({ ride, onConfirm, onCancel }) => {
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-xl p-6 max-w-sm w-full">
        ...
        <p className="mb-6 text-gray-600">
          Are you sure you want to delete ride {ride.id}? This action cannot be undone.
        </p>
...
const RideHistory = () => {
  ...
  const handleDeleteRide = () => {
    if (rideToDelete) {
      setRides(prevRides => prevRides.filter(ride => ride.id !== rideToDelete.id));
          ...
          <div className="flex justify-between items-center">
            ...
            <div className="flex items-center space-x-4">
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent>
                  {['All', 'Successful', 'Cancelled', 'Pending', 'In Progress'].map((status) => (
                    <DropdownMenuItem 
                      key={status} 
          ...
          <div className="mt-6 flex justify-between items-center">
            ...
            <div className="flex items-center space-x-4">
              <span className="text-sm text-gray-600">
                {`${(currentPage - 1) * ridesPerPage + 1}-${Math.min(currentPage * ridesPerPage, filteredRides.length)} of ${filteredRides.length}`}
              </span>
              <Pagination
