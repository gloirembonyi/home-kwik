...
import { RefundRequest } from "./page";
...
export default function ExportFilterModal({
  ...
  const convertToTimezone = (date: Date, selectedTimezone: string): Date => {
    if (selectedTimezone === "GMT+2") {
      return new Date(date.getTime() + 2 * 60 * 60 * 1000);
    }
    return date;
  };

  const formatDataForExport = (data: RefundRequest[]) => {
    return data.map((item) => ({
      ...item,
      date: format(
        convertToTimezone(new Date(item.date), timezone),
        "yyyy-MM-dd HH:mm:ss"
      ),
  ...
  const handleExport = async () => {
    try {
      ...
      const selectedRange = dateRanges
        .find((r) => r.id === dateRange)
        ?.getRange();
      const filteredData = filterDataByDate(data, selectedRange);
      const formattedData = formatDataForExport(filteredData);

      const filename = `export_${format(new Date(), "yyyy-MM-dd_HH-mm")}.csv`;
  ...
  const handleDateRangeSelect = (newRange: string) => {
                ...
                >
                  ...
                  <div className="flex items-center space-x-2">
                    <RadioGroupItem
                      ...
                      className="text-blue-600"
                    />
                    <Label htmlFor="utc" className="font-medium">
                      UTC
                    </Label>
                  </div>
                </RadioGroup>
                    ...
                    >
                      <div className="flex items-center space-x-2">
                        <RadioGroupItem
                          ...
                          className="text-blue-600"
                        />
                        <Label htmlFor={range.id} className="font-medium">
                          {range.label}
                        </Label>
                      </div>
                      {range.date && (
