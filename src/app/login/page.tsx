  ...
  const handleVerification = async (e: React.FormEvent) => {
    ...
    try {
      const action = await axios.post(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/auth/verify-login`,
        { email, loginCodeMFA }
      );
      ...
      <div className="w-full lg:w-1/2 flex flex-col p-6 lg:p-12">
        ...
        <div className="flex-1 flex flex-col justify-center max-w-md mx-auto w-full mt-8">
          ...
          <form onSubmit={handleEmailLogin} className="mt-8 space-y-6">
            ...
            {error && <p className="text-sm my-2 text-red-500 ">{error}</p>}
        ...
        <div className="relative h-full flex flex-col items-center justify-center p-12 text-center">
          <div className="bg-white/95 backdrop-blur-md rounded-2xl p-8 max-w-md shadow-xl">
            ...
            <p className="text-gray-600">
              Experience seamless authentication and unlock the full potential
              of our platform. Your security is our top priority.
            </p>
          </div>
        </div>
      </div>
